...
AI AGENT 必须按 `AGENTS.md` 与 `.tianming/SKILL.md` 的开书协议，先校验书名和目标目录，创建或修复项目根 AGENTS.md，再问 **5 个开书问题**，最后在根目录生成带内容的【第一版知识库】：
...
如果你已经在同一条消息中完整回答 5 个开书问题，AI 不应重复问；否则必须先问完再生成文件。回答后，AI 才能生成新书目录和第一版知识库。若你的回答较简略，AI 可以做最小合理假设，但必须把不确定项标为“待确认”。
...
`我的新书/世界基石.md` 是动态核心，开书时会生成初版，后续由 `「天命：大纲」`、`「天命：规划」`、`「天命：目录」`、`「天命：存档」` 逐步维护。
...
1. 先校验书名和目标目录，禁止非法路径、保留名和覆盖已有非空目录。
2. 创建或修复项目根 AGENTS.md，确保不遵守 Skill 的 AI AGENTS 也会读取 `.tianming/SKILL.md`。
3. 若用户没有在同一条消息中完整回答 5 个开书问题，先问 5 个开书问题并等待回答。
4. 从 `.tianming/kb-templates/` 读取模板结构。
5. 在 `书名/` 内生成五个含初始内容的知识库文件和 `README.md`。
6. 将 `书名/` 设为当前会话的【当前新书目录】。
